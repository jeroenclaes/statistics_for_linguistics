...
---
## 1. Case study one: *Dude* or *Duuude*?
```{r, include=FALSE}
#library(testwhat)
...
# Perform a t-test of the differences bretween surfers and non-surfers. You will need to use the formula method.
...
## 3. The influence of formal and semantic similarity on acceptability tasks
...
# Which of the two groups has the highest median Mean_RT? You will find the correct solution on the Solution tab
```
...
# Draw a density plot of the Mean_RT-column for both groups and add a red vertical line at the mean
...
# Perform a Wilcoxon tests of the differences between the two conditios. You will need to use the formula method.
...
# Relevel 'condition' so that the reference level is 'low'
dataSet$condition <- relevel(dataSet$condition, ref="syntax and semantics")

# Perform a t-test of the differences bretween surfers and non-surfers. You will need to use the formula method.
# Also, keep in mind our hypothesis: Surfers will have higher Mean_RT-values
